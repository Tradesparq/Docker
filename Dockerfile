...
# Get sources
WORKDIR /tmp
RUN wget http://nginx.org/download/nginx-1.7.6.tar.gz http://labs.frickle.com/files/ngx_cache_purge-2.1.tar.gz
RUN tar -zxvf nginx-1.7.6.tar.gz && mv nginx-1.7.6 nginx && rm nginx-1.7.6.tar.gz
RUN tar -zxvf ngx_cache_purge-2.1.tar.gz && mv ngx_cache_purge-2.1 ngx_cache_purge && rm ngx_cache_purge-2.1.tar.gz
